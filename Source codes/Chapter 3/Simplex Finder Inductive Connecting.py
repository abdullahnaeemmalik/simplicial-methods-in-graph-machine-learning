...
class AdjGraph():
    ...
    def __init__(self,
        ...
        seed_edge_pairs = []
        print("Now creating a list of seed_edge_pairs")
        for i, u in tqdm(enumerate(src), position=0, desc="i", leave=False, colour='green', ncols=200):
            v = dst[i]
        ...
        print("Now adding in vertices in the skeleton_dictionary with their indices")
        for vertex_index in tqdm(self.vertex_index_dict[0],
                                 position=1, desc="i", leave=False, colour='green', ncols=200):
            zero_skeleton_dict.update({(0, vertex_index): [vertex_index]})

        print("Now adding in edges in the skeleton_dictionary with their indices")
        for edge_index in tqdm(self.vertex_index_dict[1],
                               position=2, desc="i", leave=False, colour='green', ncols=200):
    ...
    def connectivity_update(self):
        ...
        for key in tqdm(self.zero_skeleton_dict, position=3, desc="i",
                        leave=False, colour='green', ncols=200):
            if key[0] == height:
                top_dim_zero_skeleta.append(self.zero_skeleton_dict[key])
        print('Done adding 0-skeleta for current dimension')

        # Create a list of the 0-skeleta of all 1-simplices, i.e., edges, in our current simplicial set:
        edge_zero_skeleta = []
        for key in tqdm(self.zero_skeleton_dict,
                        position=4, desc="i", leave=False, colour='green', ncols=200):
            if key[0] == 1:
                edge_zero_skeleta.append(self.zero_skeleton_dict[key])
        print('Done adding edges with their labels for current dimension')

        # Begin list of all non-degenerate boundaries of standard (k+1)-simplices in our current simplicial set:
        top_dim_boundaries = []

        # Step 1: Iterate of 0-simplices in our current simplicial set:
        for src in tqdm(self.vertex_index_dict[0],
                        position=5, desc="i", leave=False, colour='green', ncols=200):
            print("Now in the main loop")
            # Step 2: iterate over k-simplices in our current simplicial set, extracting the 0-skeleton of each:
            for zero_skel in tqdm(top_dim_zero_skeleta,
        ...
        print("updating dictionary accordingly")
        for i, zero_skeleton in tqdm(enumerate(top_dim_boundaries),
                                     position=0, desc="i", leave=False, colour='green', ncols=200):
