...
class SimplicialFinder():
    ...
    def __init__(self,
        ...
        for node in self.seed_nodes:
            listed_nodes = listed_nodes + [[node]]
        
        
        listed_edges = []
        for edge in self.seed_edge_pairs:    
            listed_edges = listed_edges + [list(edge)]
        
        self.standard_0_skeleton_dict = {0 : listed_nodes, 1:listed_edges}
    ...
    def connectivity_update(self):
        ...
        for i, zero_skeleton in enumerate(top_dim_boundaries):
            ...
            found_simplices = found_simplices + [zero_skeleton]
        if len(found_simplices) != 0:
            self.standard_0_skeleton_dict.update({new_height:found_simplices})    
